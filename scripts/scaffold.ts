...
// ═══════════════════════════════════════════════════════════════
// Template - Agent Instructions (loaded from core/templates/AGENT.md)
// ═══════════════════════════════════════════════════════════════
...
async function getAgentMdTemplate(agent: Agent): Promise<string> {
  const templatePath = join(ROOT, "core", "templates", "AGENT.md");
  try {
    const template = await readFile(templatePath, "utf-8");
    return template
      .replace(/\{\{AGENT_NAME\}\}/g, AGENT_DISPLAY_NAMES[agent])
      .replace(/\{\{AGENT_DIR\}\}/g, agent);
  } catch {
    // Fallback to inline template if file doesn't exist
    return `# 🤖 ${AGENT_DISPLAY_NAMES[agent]} - AoC 2025 Battle Royale
...
Run \`npm run sync-agent-docs\` to get the full optimization guide.
`;
  }
}

async function getSolutionJournalTemplate(day: number): Promise<string> {
...
async function scaffoldAgents(): Promise<void> {
  ...
  for (const agent of AGENTS) {
    ...
    const agentMdContent = await getAgentMdTemplate(agent);
    await writeIfNotExists(
    ...
    for (let day = 0; day <= DAYS; day++) {
      const dayPadded = day.toString().padStart(2, "0");
      const journalContent = await getSolutionJournalTemplate(day);
      await writeIfNotExists(
        join(agentDir, "notes", `solution-day${dayPadded}.md`),
        journalContent
      );
    }
