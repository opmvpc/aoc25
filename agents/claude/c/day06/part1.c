/**
 * 🎄 Advent of Code 2025 - Day 06 Part 1 - OPTIMIZED
 * Compile: clang -O3 -march=native -o part1 part1.c
 *
 * Part 1: Numbers read horizontally (one number per row, left to right)
 */
...
int main(void) {
    ...
    for (int c = 0; c < maxc; ) {
        ...
        // Part 1: one number per row (read horizontally)
        if (op == '*') {
            long long res = 1;
            for (int r = 0; r < dr; r++) {
                long long n = 0;
                for (int col = ps; col < pe && col < lens[r]; col++) {
